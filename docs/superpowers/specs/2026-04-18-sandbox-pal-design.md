---
title: sandbox-pal — Local Agent Dispatch via Claude Code Skills
status: draft
date: 2026-04-18
author: jonny (with Claude)
---

# sandbox-pal — Design Document

## 1. Overview

**sandbox-pal** is a local agent dispatch system: a set of Claude Code skills that publish implementation plans to GitHub issues and launch ephemeral Docker containers that execute those plans, open PRs, and address review feedback. Each container is a fresh Claude Code CLI process running under the user's own subscription-backed OAuth token, isolated from the host, executing a gated pipeline (adversarial plan review → TDD implementation → post-implementation review → retry on concerns) before opening a PR.

The project is a sibling to `jnurre64/claude-agent-dispatch`, which runs the same pipeline shape on self-hosted GitHub Actions runners. sandbox-pal differs in threat model and trigger mechanism: no shared infrastructure, no multi-user runners, no Actions workflows. Instead, a user-triggered skill in the user's own Claude Code session launches a container on the user's own Docker daemon (local or remote) and notifies the user when it's done.
...
> **SUPERSEDED** (2026-04-21): See `docs/superpowers/specs/2026-04-21-sandbox-pal-auth-rework.md` — §4. Content below is preserved for history.

sandbox-pal operates as **personal, individual-use infrastructure** under Anthropic's subscription-OAuth guidance. The design specifically matches Anthropic's documented endorsed patterns:
...
│  │             sandbox-pal:latest <event> <repo> <number>             │
│  │                                                                   │
...
### 5.2 Config file — `~/.config/sandbox-pal/config.env`

> **SUPERSEDED** (2026-04-21): See `docs/superpowers/specs/2026-04-21-sandbox-pal-auth-rework.md` — §5.3. Content below is preserved for history.

Path resolution:
- Linux/macOS: `$XDG_CONFIG_HOME/sandbox-pal/config.env` (fallback `~/.config/sandbox-pal/config.env`)
- Windows: `%LOCALAPPDATA%\sandbox-pal\config.env`
...
- macOS: skill tries `security find-generic-password -a $USER -s sandbox-pal-oauth -w` first; falls back to config.env
- Windows: PowerShell `Get-StoredCredential -Target sandbox-pal-oauth` first; falls back
- Linux: no auto-detection (libsecret doesn't work headless); document `pass` as opt-in via `PAL_CRED_SOURCE=pass:sandbox-pal/oauth`
...
Location:
- Linux/macOS: `$XDG_DATA_HOME/sandbox-pal/runs/` (fallback `~/.local/share/sandbox-pal/runs/`)
- Windows: `%LOCALAPPDATA%\sandbox-pal\runs\`
...
- Linux: `notify-send "sandbox-pal" "<message>" --icon=...`
- macOS: `osascript -e 'display notification "<message>" with title "sandbox-pal"'`
- Windows: `powershell -NoProfile -Command "New-BurntToastNotification -Text 'sandbox-pal', '<message>'"` (requires BurntToast module; documented in install guide)
...
> **SUPERSEDED** (2026-04-21): See `docs/superpowers/specs/2026-04-21-sandbox-pal-auth-rework.md` — §5.2. Content below is preserved for history.
...
Projects with specific toolchain needs (Godot + GdUnit4, .NET SDK, Mono for .NET Framework, etc.) extend the base image. Pattern: repo ships a `.pal/Dockerfile.extra` that `FROM sandbox-pal:latest` and layers project-specific installs. The skill detects presence of this file at build time and produces a per-repo tag (`sandbox-pal:<repo>`).
...
> **SUPERSEDED** (2026-04-21): See `docs/superpowers/specs/2026-04-21-sandbox-pal-auth-rework.md` — §6. Content below is preserved for history.
...
- The label state machine — sandbox-pal has no state machine because each container is a single linear run.
...
- Shared config between sandbox-pal and claude-agent-dispatch for projects that use both
...
`sandbox-pal` was chosen after collision checks against `claude-buddy` (heavy collision: Anthropic's `/buddy` April 1 2026 pet feature, existing npm package, existing competing product at `claude-buddys.com`), `claude-sidekick` (crowded: 6+ active repos, multiple commercial products including Shopify and monday.com using "sidekick" for AI dev assistants, Anthropic's own marketing calling Claude Code a "coding sidekick"), `claude-ditto`/`claude-shadow`/`claude-partner` (all major collisions including Anthropic's official Partner Network for the last), and `claude-doppelganger` (clean namespace but poor CLI ergonomics — long, hard to spell).

`sandbox-pal` has minor residual signals (two recently-pushed same-name repos in different niches, `claudepal.com` redirect-squatted to an unrelated marketing tool), all non-blocking. Grab repo + `sandbox-pal.app` domain before additional discovery noise accumulates.
